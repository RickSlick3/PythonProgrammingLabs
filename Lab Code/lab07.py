...
class Player(object):
    ...
    def go_to(self, direction):
        ...
        >>> bbc = Place('Bearcat Cafe', 'You are at Bearcat Cafe', [], [])
        >>> swift.add_exits([bcc])
        >>> bcc.add_exits([swift])
    ...
    def talk_to(self, person):
        ...
        """
        if type(person) != str:
            return print('Person has to be a string.')
        for x in self.place.characters:
            if x == person:
                print(person + " says: " + self.place.characters[x].talk())
                return 
    ...
    def take(self, thing):
        ...
        """
        if type(thing) != str:
            return print('Thing should be a string.')
        for x in self.place.things:
            if x == thing:
                self.backpack += [self.place.take(x)]
                return print(self.name + " takes the " + thing)
