...
_credits_ = ["collaborated with Andrew McCrary RQ#4"]
_email_ = "Roberrf@mail.uc.edu"
    ...
    i_love_music
      ...
      trance
        darude
          sandstorm
    """
    return tree(username, [tree('pop', [tree('justin bieber', [tree('single', [tree('what do you mean?')])]), 
                                tree('2015 pop mashup')]), tree('trance', [tree('darude', [tree('sandstorm')])])])
...
def num_songs(t):
    ...
    if is_leaf(t):
        ...
        branch = [num_songs(b) for b in branches(t)]
        return sum(branch)
...
def add_song(t, song, category):
    ...
    if root(t) == category:
        ...
        return tree(root(t), [add_song(branches(t)[0], song, category)])
    elif len(branches(t)) == 2:
        return tree(root(t), [add_song(branches(t)[0], song, category), add_song(branches(t)[1], song, category)])
    else:
        return tree(root(t))
...
def delete(t, target):
    ...
    # if target in [root(x) for x in branches(t)]:
